...
def simulate_test
  ...
  @test_logs.each do |line|
    ...
    puts ("\n##########################################################\n")
    puts("REQUEST: \n curl -L --max-redirs 20 -v -i -H \'Accept: application/json\' -X #{split[0]} http://#{@username}:#{@key}@#{@hub}/wd/hub#{split[1]} -d \'#{json.strip}\'") 
    response = `curl -L --max-redirs 20 -v -i -H \'Accept: application/json\' -X #{split[0]} http://#{@username}:#{@key}@#{@hub}/wd/hub#{split[1]} -d \'#{json.strip}\'`
    puts ("\n----------------------------------------------------------\n")
    puts ("RESPONSE \n #{response}")
  end
  end_time = Time.now 
